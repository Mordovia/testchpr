...
function wallGet(){
	...
	VK.api('wall.get', {
		...
		document.getElementById('newPrognoz').innerHTML = '<table><tr><td><input value = "Перейти к записи" type = "button" onclick = look();></td><td><input type="datetime-local"></td><td><input value = "Опубликовать" type = "button" onclick = "postIt();"></td><td><input value = " >> Получить другую запись" type = "button" onclick = "newPost();"></td></tr></table>';
		backToMainMenu();
}
function newPost(){
//path = '';
